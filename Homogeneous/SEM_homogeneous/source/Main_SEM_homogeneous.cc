...
#include "Param_handler.h"
  ...
  template <int dim>
  class SEM_homogeneous
  {
  public:
    SEM_homogeneous (const struct code_parameters & parameters);
    void run ();
...
// Constructor
  template <int dim>
  SEM_homogeneous<dim>::SEM_homogeneous (const struct code_parameters & parameters) :
...
//    x_end (parameters.x_end),
  ...
  template <int dim>
  void SEM_homogeneous<dim>::setup_system ()
  {
   ...
   assembly_timer.start();
       {
           ...
           for (; cell!=endc; ++cell)
           {
               ...
               // updating global mass vector, and laplace matrix
...
//    std::ofstream laplace_out("laplace.txt");
//    std::ofstream mass_out("mass.txt");
//    std::ofstream boundary_out("boundary.txt");
//    laplace_matrix.print(laplace_out);
//    mass_matrix.print(mass_out);
//    boundary_matrix.print(boundary_out);
...
//***********************************************
 ...
 factor_timer.start();
    ...
    //inverse of vector_u1 and vector_un
  ...
  template <int dim>
  void SEM_homogeneous<dim>::output_results () const
  {
  ...
  try
    {
      using namespace dealii;
      //argv[1]
      //const char file[]="./../input_sem_homogeneous.prm";
      ParameterReader read_param (argv[1]);
      code_parameters parameters =read_param.get_parameters();

      SEM_homogeneous<2> wave_equation_solver(parameters);
