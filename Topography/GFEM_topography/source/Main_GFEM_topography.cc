...
#include "Param_handler.h"
  ...
  template <int dim>
  class GFEM_topography
  {
  public:
    GFEM_topography (const struct code_parameters & parameters);
    ~GFEM_topography ();
    void run ();
...
// Constructor
  ...
  template <int dim>
  GFEM_topography<dim>::GFEM_topography (const struct code_parameters & parameters) :
   ...
   */
  template <int dim>
  GFEM_topography<dim>::~GFEM_topography()
  {
    dof_handler.clear ();
  }

  template <int dim>
  void GFEM_topography<dim>::setup_system ()
  {
    //timer start
    ...
    const std::string mesh_name="topo_meshr2_neg.msh";
    if (mesh_name.compare(input_mesh)==0)
        {
            const unsigned int mid=12;
            mesh.Mesh_refine(triangulation,mid);
        }
...
//                std::sqrt (1.*dim);
    ...
    assembly_timer.start();
      {
          ...
          for (; cell!=endc; ++cell)
          {
              ...
              //query if cell is at boundary, then compute the local boundary matrix
              //id==10 refers to all boundary faces except top boundary faces
  ...
  template <int dim>
  void GFEM_topography<dim>::solve_u ()
  {
  ...
  template <int dim>
  void GFEM_topography<dim>::solve_v ()
  {
  ...
  template <int dim>
  void GFEM_topography<dim>::output_results () const
  {
  ...
  try
    {
      using namespace dealii;
      //argv[1]
      //const char file[]="./../input_gfem_topography.prm";
      ParameterReader read_param (argv[1]);
      code_parameters parameters =read_param.get_parameters();

      GFEM_topography<2> wave_equation_solver(parameters);
