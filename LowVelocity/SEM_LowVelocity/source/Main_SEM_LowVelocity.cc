...
#include "Param_handler.h"
  ...
  template <int dim>
  class SEM_LowVelocity
  {
  public:
    SEM_LowVelocity (const struct code_parameters & parameters);
    void run ();
...
// Constructor
  template <int dim>
  SEM_LowVelocity<dim>::SEM_LowVelocity (const struct code_parameters & parameters) :
  ...
  template <int dim>
  void SEM_LowVelocity<dim>::setup_system ()
  {    
...
//*** non time -dependent matrices and vectors
 ...
 factor_timer.start();
  ...
  template <int dim>
  void SEM_LowVelocity<dim>::output_results () const
  {
  ...
  try
    {
      using namespace dealii;
      //argv[1]
      //const char file[]="./../input_sem_LowVelocity.prm";
      ParameterReader read_param (argv[1]);
      code_parameters parameters =read_param.get_parameters();

      SEM_LowVelocity<2> wave_equation_solver(parameters);
